...
namespace Resource_Manager.Classes.L33TZip
{
	public static class L33TZipUtils
	{
		...
		#region Compressing
		public static async Task<byte[]> ExtractL33TZippedBytesAsync(string inputFileName)
		{
		...
		public static async Task CompressBytesAsL33TZipAsync(
			byte[] inputBytes,
		...
		{
			return ExtractL33TZippedBytesAsync(inputFileName).GetAwaiter().GetResult();
		}

		[Obsolete("Use CompressFileAsL33TZipAsync instead")]
		public static Task DoCreateL33TZipFile(
		...
		{
			ExtractL33TZipFileAsync(inputFileName, outputFileName).GetAwaiter().GetResult();
		}

		[Obsolete("Use ExtractL33TZipFileAsync instead")]
		public static byte[] ExtractL33TZipFile(string zipFileName)
		{
			return ExtractL33TZipFileAsync(zipFileName).GetAwaiter().GetResult();
		}

		[Obsolete("Use ExtractL33TZippedBytesAsync instead")]
		public static byte[] ExtractL33TZipFile(byte[] zipData)
		{
			return ExtractL33TZippedBytesAsync(zipData).GetAwaiter().GetResult();
		}

		[Obsolete("Use ExtractL33TZipStreamAsync instead")]
		public static byte[] ExtractL33TZipFile(BinaryReader zipReader)
		{
			return ExtractL33TZipStreamAsync(zipReader).GetAwaiter().GetResult();
		}

		[Obsolete("Use ExtractL33TZipFileAsync instead")]
		public static Task DoExtractL33TZipFile(
			string fileName,
			string outputFileName,
			CancellationToken ct = default,
			IProgress<double> progress = null
