	...
	public partial class MainWindow : Window, INotifyPropertyChanged
	{
		...
		public MainWindow()
		{
			...
			for (int i = 0; i < Math.Min(10, Settings.Default.RecentFiles.Count); i++)
				...
				{
					FileName = Settings.Default.RecentFiles[i],
					Title = Path.GetFileName(Settings.Default.RecentFiles[i]),
					OnClickCommand = new RelayCommand<string>(openFile)
				});
		...
		private async void files_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			...
			try
			{
				if (files.SelectedItem is not BarEntry entry)
				{
		...
		private void Thumb_DragDelta(object sender, DragDeltaEventArgs e)
		{
			...
			if (senderAsThumb.TemplatedParent is not GridViewColumnHeader header) return;
		...
		private async void ExtractMenuItem(object sender, RoutedEventArgs e)
		{
				...
				if (openFileDialog.ShowDialog() == true)
				{
					...
					await Task.Run(() =>
					{
						entries = File.
							BarFile.
							BarFileEntries.
							Where(x => icons.Any(y => x.FileNameWithRoot.ToLower().EndsWith(y.Replace('/', '\\').ToLower()))).
							ToList();
					});
				}
			}
			...
			if (entries.Count != 0)
			{
				...
				ExtractDialog ExtractDialog = new(File.BarFilePath);
				if (ExtractDialog.ShowDialog() == true)
				{
					...
					try
					{
						...
						{
							await File.saveFiles(
								entries,
								RootPath,
								decompress,
								Token,
								ExtractDialog.AutoDDTToPNGConversion,
								ExtractDialog.AutoDDTToTGAConversion,
								ExtractDialog.AutoXMBConversion,
								ExtractDialog.OneFolder,
								ExtractDialog.SavePNGasBMP,
								ExtractDialog.AutoJSONConversion,
								ExtractDialog.OverlayColor,
								ExtractDialog.CompressPNG,
								ExtractDialog.AutoPNGToWEBPConversion
							);
						});
		...
		{
			// await DDTFileUtils.
			// 	DDT2PngAsync(@"D:\Development\Resource_Manager\Resource_Manager\bin\Release\netcoreapp3.1\Art\ui\alerts\alert_treatyend_bump.ddt");
		}
		...
		private async void MenuItem_Click_7(object sender, RoutedEventArgs e)
		{
			...
			CreateBarFileDialog createBarFileDialog = new();
			if (createBarFileDialog.ShowDialog() == true)
			{
				try
				{
					...
					NotifyPropertyChanged("recentFiles");
					NotifyPropertyChanged("file");
					File = await BarViewModel.Create(
						createBarFileDialog.RootPath,
						createBarFileDialog.Version,
						createBarFileDialog.BarFileName
					);
					...
					recentFiles.Insert(0, new RecentFile()
					{
						FileName = File.BarFilePath,
						Title = Path.GetFileName(File.BarFilePath),
						OnClickCommand = new RelayCommand<string>(openFile)
					});
				...
				if (openFileDialog.ShowDialog() == true)
				{
					...
					foreach (var file in openFileDialog.FileNames)
					{
						...
						catch (Exception ex)
						{
							MessageBox.Show(
								ex.Message,
								$"Conversion error - {Path.GetFileName(file)}",
								MessageBoxButton.OK,
		...
		private async void MenuItem_Click_8(object sender, RoutedEventArgs e)
		{
			...
			var json = JsonSerializer.Serialize(File.BarFile.BarFileEntries, options);
